...
namespace FoxOne.Core
{
    ...
    /// 获取汉字的首字母和全拼  
    /// </summary>  
    public static class ChineseCode
    {
        public static string GetAccount(string name)
        {
            string firstLetter = GetSpell(name[0].ToString());
            string secondLetter = GetOneIndex(name[1].ToString());
            if (name.Length == 3)
            {
                return firstLetter + secondLetter + GetOneIndex(name[2].ToString());
            }
            else
            {
