...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '有些文件未被添加到上传队列:';
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							...
							this.queueData.errorMsg += '\n选择的文件数超过上传队列限制 (' + settings.queueSizeLimit + ').';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件： "' + file.name + '" 超过限制大小 (' + settings.fileSizeLimit + ').';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n文件： "' + file.name + '" 是空的.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件： "' + file.name + '" 是一个不可访问的类型 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('文件名为 "' + file.name + '" 的文件已在服务器中存在.\n你是否希望替换该文件?');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
	...
	$.fn.uploadify = function(method) {
		...
		if (methods[method]) {
			...
			$.error('方法名为 ' + method + ' 的方法不存在于 $.uploadify');
		}
